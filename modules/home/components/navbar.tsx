...
function Navbar({ userRole }: { userRole: UserRole | null }) {
  return (
    <nav className="fixed top-4 left-1/2 transform -translate-x-1/2 z-50 w-full max-w-5xl px-4">
      <div className="bg-secondary/5 backdrop-blur-md border border-border rounded-2xl shadow-lg shadow-black/5 dark:shadow-black/20 transition-all duration-200 hover:bg-secondary/10">
        <div className="px-6 py-4 flex justify-between items-center">
          ...
          <div className="flex flex-row items-center justify-center gap-x-4">
            <Link
              href="/problems"
              className="text-sm font-medium text-secondary-foreground hover:text-primary"
            >
              Problems
            </Link>
            <Link
